...
function listar() {
    ...
    var instrucao = `
        SELECT u.id,u.email,u.senha,u.fkTipoUsuario,u.fkFuncionario, t.tipoUsuario, f.nome, f.fkEmpresa FROM usuario u join tipoUsuario t on u.fkTipoUsuario = t.id join funcionario f on u.fkFuncionario = f.id;
    `;
...
function pegarUsuario(email) {
    var instrucao = `
    SELECT u.id,u.email,u.senha,u.fkTipoUsuario,u.fkFuncionario, t.tipoUsuario, f.nome FROM usuario u join tipoUsuario t on u.fkTipoUsuario = t.id join funcionario f on u.fkFuncionario = f.id WHERE email = '${email}';
    `;
    console.log("Executando a instrução SQL: \n" + instrucao);
    return database.executar(instrucao);
}
